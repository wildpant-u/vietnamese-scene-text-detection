...
def print_draw_crop_rec_res( img_crop_list, img_name):
        ...
        for bno in range(bbox_num):
          ...
          crop_name_w = "./PaddleOCR/train_data/vietnamese/rec/test_img_crop/{}".format(crop_name)
          cv2.imwrite(crop_name_w, img_crop_list[bno])
          crop_label.write("{0}\t{1}\n".format(crop_name, text[bno]))

crop_label = open('./PaddleOCR/train_data/vietnamese/rec/test_crop_label.txt','w', encoding='utf-8')
with open('./PaddleOCR/train_data/vietnamese/text_localization/test_label_viet.txt','r', encoding='utf-8') as file_text:
...
for img_file in img_files:
  ...
  image_file = './PaddleOCR//train_data/vietnamese/text_localization/test_images/' +img_file.split('\t')[0]
